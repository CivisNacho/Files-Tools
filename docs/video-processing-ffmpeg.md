...
- `SoftMux`: keeps the subtitle as a separate (non-burned) track, never burning and never lossily transcoding
  styled ASS. Behaviour by container:
  - **MKV**: the subtitle is embedded as a self-contained soft track. A subtitle already in a Matroska-native
    format (`.ass`/`.ssa`/`.srt`/`.vtt`) is **stream-copied** (`-c:s copy`), not re-encoded — re-encoding ASS
    via `-c:s ass` simplifies override tags and mangles styled, one-event-per-word karaoke. See
    `ResolveSubtitleEmbedCodec`.
  - **MP4/MOV/WebM with `.ass`/`.ssa`**: ASS styling can't be carried, so the video is left untouched and the
    styled subtitle is written as a **sidecar** beside the output (matching base name, e.g. `myvideo.ass` next
    to `myvideo.mp4`) for the player to load via libass. See `ShouldSidecarAssSubtitle` /
    `WriteSidecarSubtitleIfNeeded`.
  - Non-ASS subtitles (e.g. `.srt`) are embedded as a soft track where the container supports it
    (`mov_text` for MP4, native for MKV/WebM).
