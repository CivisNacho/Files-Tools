...
- `SoftMux`: keeps the subtitle as a separate (non-burned) track. For a styled `.ass`/`.ssa` source the video
  is left untouched and the subtitle is written as a **sidecar** beside the output (matching base name, e.g.
  `myvideo.ass` next to `myvideo.mp4`/`myvideo.mkv`) so players load it via libass. This is done for **every**
  container — including MKV: although MKV can embed an ASS track natively, many players render an *embedded*
  ASS track with a basic text renderer (mangling styled, one-event-per-word karaoke into unstyled flashing
  text), whereas an external sidecar renders correctly. SoftMux never burns and never transcodes ASS to a
  lossy codec — styling and karaoke animation are preserved. Non-ASS subtitles (e.g. `.srt`) are still embedded
  as a soft track where supported. See `ShouldSidecarAssSubtitle` / `WriteSidecarSubtitleIfNeeded`.
